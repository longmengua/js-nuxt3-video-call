...
const ws = new WebSocket(config.public.WEBSOCKET_URL);
...
const joinRoom = () => {
  if (roomName.value === '') {
    ...
    ws.send(JSON.stringify({ type: 'join', room: roomName.value }));
  }
};

const OnIceCandidateFunction = (event) => {
  if (event.candidate) {
    ws.send(JSON.stringify({ type: 'candidate', candidate: event.candidate, room: roomName.value }));
  }
};
...
ws.onmessage = async (message) => {
  ...
  switch (data.type) {
    ...
    case 'joined':
      state.creator = false;
      await setupStream();
      ws.send(JSON.stringify({ type: 'ready', room: roomName.value }));
      break;
    case 'full':
    ...
    case 'ready':
      if (state.creator) {
        ...
        state.rtcPeerConnection
          ...
          .then((offer) => {
            ...
            ws.send(JSON.stringify({ type: 'offer', offer: offer, room: roomName.value }));
          })
          .catch((error) => {
            console.log(error);
          });
      ...
      break;
    case 'candidate':
      state.rtcPeerConnection.addIceCandidate(data.candidate);
      break;
    case 'offer':
      state.rtcPeerConnection = new RTCPeerConnection(iceServers);
      state.rtcPeerConnection.onicecandidate = OnIceCandidateFunction;
      ...
      state.rtcPeerConnection.addTrack(state.userStream.getTracks()[0], state.userStream);
      state.rtcPeerConnection.addTrack(state.userStream.getTracks()[1], state.userStream);
      state.rtcPeerConnection.setRemoteDescription(new RTCSessionDescription(data.offer));
      state.rtcPeerConnection
        .createAnswer()
        .then((answer) => {
          state.rtcPeerConnection.setLocalDescription(answer);
          ws.send(JSON.stringify({ type: 'answer', answer: answer, room: roomName.value }));
        })
        .catch((error) => {
          console.log(error);
        });
      break;
    case 'answer':
      state.rtcPeerConnection.setRemoteDescription(new RTCSessionDescription(data.answer));
      break;
  }
};
  ...
  <div>
    ...
    <div :style="{display: !uiState.isOpenRoom ? 'none' : 'block'}">
      <div class="video-group">
        <div class="video-shared-group" :style="{ position: 'relative', width: `${uiState.videoSharing.width}px`, height: `${uiState.videoSharing.height}px` }">
          <video
            class="screen-video"
            ref="screenVideoRef"
            autoplay
